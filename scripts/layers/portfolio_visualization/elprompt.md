...
- USD CCL marca la cuenta corriente en dolares cable
...
Me vas a ayudar a desarrollar el código de programación en lenguaje Python y vamos a iniciar analizando si la siguiente lógica para las métricas que se tienen que calcular tiene sentido:
```json
{
    ...
    "LICITACION PRIVADA": "Disminuye el valor en Cash_Total_USD y lo aumenta en Total_Growth_Valuation o Total_Safe_Valuation dependiendo de la especie",
    "LICITACION PARIDAD": "Disminuye el valor en Cash_Total_USD y lo aumenta en Total_Safe_Valuation o Total_Growth_Valuation dependiendo de la especie",
    ...
    "NOTA DE CREDITO U$S": "Aumenta el valor en Cash_Total_USD (Movimiento de Canje, el efecto cruzado neto en Patrimonio_USD debe ser 0)",
    "NOTA DE DEBITOS U$S": "Disminuye el valor en Cash_Total_USD (Movimiento de Canje, el efecto cruzado neto en Patrimonio_USD debe ser 0)",
    "RETENCION DOLAR MEP": "Disminuye el valor en Cash_Total_USD",
    "COMPRA CAUCION CONTADO": "Disminuye el valor en Cash_Total_USD y lo aumenta en Total_Safe_Valuation",
...
Es necesario para la implementación pensar en una estructura de datos que me permita facilmente poder calcular las siguientes métrias:
- Operado: Fecha en la que se realizo la operacion.
- Cash_Total_USD: Importe liquido.
- Total_Safe_Valuation: Valor total en dolares de los activos de renta fija (Bonos Soberanos, Obligaciones Neociables, Cauciones)
- Total_Growth_Valuation: Valor total en dolares de los activos de renta variable (Acciones Argentinas, Acciones del exterior, ETFs del exterior, Fondos Comunes de Inversion Abiertos, CeDeArs)
...
on hp.date = cm."date" ;
```
Teniendo en cuenta que el si el ticker esta dentro de este diccionario:
```
ratios_cedear = {
            'KO': 5.0, 'SPY': 20.0, 'QQQ': 20.0, 'AAPL': 10.0,
            'GOOGL': 58.0, 'MSFT': 30.0, 'TSLA': 15.0, 'MELI': 120.0,
...
```
Y la columna "source" es diferente de "YFinance_USD" hay que multiplicar el valor de close_usd por el valor de la clave que le corresponde a la especie.

Para el caso de fondos comunes de iversión abiertos como ALGIIA, BMACTAA, BULL-IA, BULMAAA y RIGAHOR, al no tener cotización de precios historico dentro de mi base de datos, habrá que hacer una excepción a las reglas anteriores. Dado que se deberá mantener el importe dentro de Total_Growth_Valuation y en caso de hacer un rescate, restar el importe original de Total_Growth_Valuation y sumar el nuevo importe del rescate a Cash_Total_USD
