...
def gen_report(df_securities, broker_name):
    ...
    df_securities_first_day = df_securities[df_securities['partition_date'] == profit_dict['first_partition_date']]
    df_securities_last_day = df_securities[df_securities['partition_date'] == profit_dict['last_partition_date']]
    ...
    profit_dict['best_securitie_profit'] = df_report["ars_profit_diff"].max()
    profit_dict['worst_securitie'] = df_report["ticket"].iloc[df_report["ars_profit_diff"].idxmin()]
    profit_dict['worst_securitie_profit'] = df_report["ars_profit_diff"].min()
    ...
    profit_dict['ars_profit_first'] = df_report['ars_profit_first'].sum()

    profit_dict['ars_profit_last'] = df_report['ars_profit_last'].sum()

    profit_dict['ars_profit_diff'] = profit_dict['ars_profit_last'] - profit_dict['ars_profit_first']

    profit_dict['percentage_profit_diff'] = ((profit_dict['ars_profit_last'] - profit_dict['ars_profit_first']) / profit_dict['ars_profit_first']) * 100

    return profit_dict

def gen_total_profit(df_reports, context):

    context['first_proffit'] = df_reports['ars_profit_first'].sum()
    context['last_proffit'] = df_reports['ars_profit_last'].sum()
    context['proffit_difference'] = context['last_proffit'] - context['first_proffit']
    context['proffit_difference_percentage'] = ((context['last_proffit'] - context['first_proffit']) / context['first_proffit']) * 100
...
def gen_pdf_report(context, template_html_report_path, output_pdf_report_path):
    ...
    # template = env.get_template(template_html_report_path)
...
def main(**kwargs):
    ...
    bullma_query_profit_per_securitie = set_queries(workings_days_week, 'Bull Market', kwargs.get('partition_date_last'), kwargs.get('partition_date_first'))
    iol_query_profit_per_securitie = set_queries(workings_days_week, 'Invertir Online', kwargs.get('partition_date_last'), kwargs.get('partition_date_first'))
    ...
    gen_pdf_report(context, kwargs.get('template_html_report_path'), kwargs.get('output_pdf_report_path'))
